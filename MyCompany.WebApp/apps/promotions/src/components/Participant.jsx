...
const Participant = () => {
    ...
    const filteredData = participants.filter(p =>
        (p.codParticipant || p.CodParticipant || '').toString().toLowerCase().includes(searchTerm.toLowerCase()) ||
        (p.codHier || p.CodHier || '').toString().toLowerCase().includes(searchTerm.toLowerCase())
    );
